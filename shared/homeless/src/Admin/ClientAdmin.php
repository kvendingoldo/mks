...
use App\Form\DataTransformer\ImageStringToFileTransformer;
...
class ClientAdmin extends BaseAdmin
{
    ...
    public function processFieldValues($object, $isUpdate = false)
    {
        ...
        $fieldRepo = $em->getRepository('App:ClientField');
        $fieldValueRepo = $em->getRepository('App:ClientFieldValue');
    ...
    public function isMenuItemEnabledShelterHistory($client)
    {
        return !!$this->getConfigurationPool()->getContainer()->get('doctrine.orm.entity_manager')
            ->getRepository('App:ShelterHistory')->findOneBy(['client' => $client]);
    }
    ...
    private function getFieldOptionValueId($valueName, $field)
    {
        ...
        $fieldOptionRepo = $em->getRepository('App:ClientFieldOption');
    ...
    public function getTemplate($name)
    {
        switch ($name) {
            case 'show':
                $name = 'App:Admin\Client:base_show.html.twig';
                break;
            case 'edit':
                $name = 'App:Admin\Client:base_edit.html.twig';
                break;
            default:
